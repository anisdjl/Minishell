...
/*   Updated: 2026/04/08 17:56:23 by adjelili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	write_in_file(t_tree *node, t_env *env, t_redir *redir)
{
	...
	(sigemptyset(&sa.sa_mask)), (sa.sa_handler = &handler_heredoc);
	sa.sa_flags = 0;
	(sigaction(SIGINT, &sa, NULL)), (sa.sa_handler = SIG_IGN);
	...
	while (1)
	{
		...
		if (exit == 1 || exit == -2)
			break ;
		if (write_in_file_check(line, redir, env))
			break ;
	}
