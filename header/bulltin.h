...
/*   Updated: 2026/04/07 17:01:21 by eprieur          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						exit_command_pipe(t_tree *node, t_env *env,
								int *fd_in, int *fd_out);
int							pwd_command_pipe(t_tree *node, t_env *env,
								int *fd_in, int *fd_out);
int							cd_command_pipe(t_tree *node, t_env *env,
								int *fd_in, int *fd_out);
int							env_command_pipe(t_tree *node, t_env **env,
								int *fd_in, int *fd_out);
int							echo_command_pipe(t_tree *node, t_env *env,
								int *fd_in, int *fd_out);
int							echo_command2_pipe(t_tree *node, t_env *env,
								int *fd_in, int *fd_out);
int							unset_command_pipe(t_tree *node, t_env **env,
								int *fd_in, int *fd_out);
void						exit_non_numeric_pipes(char **arg, t_env *env);
int							env_command_for_export_pipe(t_tree *node,
								t_env **env, int *fd_in, int *fd_out);
int							export_pipe(t_tree *node, t_env **env, int *fd_in,
								int *fd_out);
void						numeric_exit_pipes(char **arg, char *nptr,
								t_env *env);
int							builtin_pipe(t_tree *node, t_env *env, int *fd_in,
								int *fd_out);
void						extract_oldpwd(t_value_node *n_value, t_env *env,
								char *new_value);
...
void						exit_non_numeric(char **arg, t_env *env, int *fd_in,
								int *fd_out);
...
void						export_update_create(t_tree *node, t_env **env,
								int y);
void						create_new_node(t_env **env, char *arg);
int							go_to_home(t_tree *node, t_env *env,
								char *current_dir);
int							pwd_command_loop(t_env *tmp, void *ptr,
								char *current_dir);
int							return_to_old_pwd(t_tree *node, t_env *env,
								char *current_dir);
int							go_to_home_pipe(t_tree *node, t_env *env,
								char *current_dir);
void						extract_oldpwd_pipe(t_value_node *n_value,
								t_env *env, char *new_value);
int							return_to_old_pwd_pipe(t_tree *node, t_env *env,
								char *current_dir);
int							error_cd_pipe(char **arg, int *fd_in, int *fd_out);
void						remove_env_node(t_env **env, t_env *current,
								t_env *prev);
